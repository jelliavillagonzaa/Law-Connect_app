...
/// Staff onboarding: apply → applicant may complete email OTP before or after admin approves → admin activates user.
...
class StaffApplicationService {
  ...
  /// Serializes [FieldValue] and other non-JSON values so backups restore cleanly.
  static Map<String, dynamic> _dataForBackup(Map<String, dynamic> raw) {
  ...
  Future<Map<String, dynamic>> approveApplication({
    ...
    try {
      ...
      final preRegisteredUid = (data?['registeredUid'] as String?)?.trim();

      final newStatus =
          (preRegisteredUid != null && preRegisteredUid.isNotEmpty)
              ? 'registered'
              : 'approved';
      ...
      if (preRegisteredUid != null && preRegisteredUid.isNotEmpty) {
        batch.update(_firestore.collection('users').doc(preRegisteredUid), {
          'assignedAttorneyId': assignedAttorneyId,
      ...
      final message = (preRegisteredUid != null && preRegisteredUid.isNotEmpty)
          ? 'Application approved. This staff member can sign in now.'
          : 'Application approved. They can complete registration with this email.';
  ...
  /// Removes the application after copying it to [backups] (admin only).
    ...
    try {
      ...
      if (data != null) {
        await BackupService().backupBeforeDelete(
          collection: collectionName,
          documentId: docId,
          data: _dataForBackup(data),
        );
      }
      await ref.delete();
      return {'success': true, 'message': 'Application removed. You can recover it from Backup.'};
    } catch (e) {
