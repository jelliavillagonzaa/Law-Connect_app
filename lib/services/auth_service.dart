...
class AuthService {
  ...
  /// Demo / firm emails map to the correct portal even if Firestore `role` is wrong.
  static String resolvePortalRole(String email, {String? firestoreRole}) {
    final e = email.trim().toLowerCase();
    if (e.isEmpty) return (firestoreRole ?? 'client').toLowerCase();

    if (e == 'staff@gmail.com' ||
        e.startsWith('staff@') ||
        (e.startsWith('staff') && e.contains('@'))) {
      return 'staff';
    }
    if (e == 'admin@gmail.com' ||
        e.startsWith('admin@') ||
        e.contains('@admin.') ||
        (e.contains('admin') && e.contains('@'))) {
      return 'admin';
    }
    if (e == 'attorney@gmail.com' ||
        e.contains('attorney') ||
        e.contains('lawyer') ||
        e.contains('@attorney')) {
      return 'attorney';
    }
    return (firestoreRole ?? 'client').toLowerCase();
  }

  /// Fixes `users/{uid}.role` when email clearly belongs to staff/attorney/admin.
  Future<void> repairPortalRoleIfNeeded({
    required String uid,
    required String email,
    String? firestoreRole,
  }) async {
    final resolved = resolvePortalRole(email, firestoreRole: firestoreRole);
    final stored = (firestoreRole ?? 'client').toLowerCase();
    if (resolved == stored) return;
    if (!['admin', 'attorney', 'staff'].contains(resolved)) return;
    ...
    try {
      ...
      if (kDebugMode) {
        debugPrint('✅ Repaired users/$uid role: $stored → $resolved');
      }
    } catch (e) {
      if (kDebugMode) debugPrint('repairPortalRoleIfNeeded: $e');
    }
  }

  Future<void> _syncEmailVerifiedFromAuth(User user, bool wasVerified) async {
    try {
      await user.reload();
      final reloaded = _auth.currentUser;
      if (reloaded != null && reloaded.emailVerified && !wasVerified) {
      ...
      if (kDebugMode) debugPrint('⚠️ Verification sync skipped: $e');
    }
  }

  Future<void> _postLoginSideEffects(String uid, String role) async {
    try {
      await FCMService().saveTokenForUser(uid);
    } catch (e) {
      if (kDebugMode) debugPrint('⚠️ FCM token save after login: $e');
    }
    try {
      ...
      if (kDebugMode) debugPrint('⚠️ Login audit log: $e');
    }
  }
  ...
  Future<Map<String, dynamic>?> syncStaffRoleFromStaffApplication({
    ...
    try {
      final appDocId = StaffApplicationService.applicationDocIdForEmail(
        emailNorm,
      );
      ...
      final isStaffRole =
          currentRole == 'staff' ||
          currentRole.contains('paralegal') ||
          currentRole.contains('assistant');

      if (isStaffRole) {
        final isInactive = data['isActive'] == false;
        final appReady = status == 'approved' || status == 'registered';
        final uidMatches = regUid == null || regUid.isEmpty || regUid == uid;
        final needsActivation = isInactive && appReady && uidMatches;
        final attorneyMismatch =
            attorneyId.isNotEmpty && currentAttorney != attorneyId;

        if (needsActivation || attorneyMismatch) {
          final update = <String, dynamic>{
            'updatedAt': FieldValue.serverTimestamp(),
          };
          if (needsActivation) {
            update['isActive'] = true;
            update['staffAwaitingApplicationApproval'] = FieldValue.delete();
            update['staffPendingAdminActivation'] = FieldValue.delete();
          }
          if (attorneyMismatch) {
            update['assignedAttorneyId'] = attorneyId;
          }
          await _firestore.collection('users').doc(uid).update(update);
          if (kDebugMode && needsActivation) {
            debugPrint(
              '✅ Activated staff user from approved application: $emailNorm',
            );
          }
          final ret = <String, dynamic>{'role': 'staff', 'isVerified': true};
          if (needsActivation) ret['isActive'] = true;
          if (attorneyMismatch && attorneyId.isNotEmpty) {
            ret['assignedAttorneyId'] = attorneyId;
          }
          return ret;
        }
        return null;
      ...
      final patch = <String, dynamic>{
        ...
        'isVerified': true,
        'isActive': true,
        'staffAwaitingApplicationApproval': FieldValue.delete(),
        'staffPendingAdminActivation': FieldValue.delete(),
        'updatedAt': FieldValue.serverTimestamp(),
      };
  ...
  Future<Map<String, dynamic>> clientSignup({
    ...
    try {
      ...
      // Create Firebase Auth user first (OTP SMS uses PhilSMS on the next screen).
      ...
      // OTP is sent after the user chooses Email or SMS on the sign-up screen.
      return {
        'success': true,
        'message':
            'Account created. Choose where to receive your verification code.',
        'email': email,
        'otp': otp, // For dev/debug display on OTP screen when enabled
        'userId': userId,
      };
    } catch (e) {
      if (kDebugMode) {
  ...
  /// Sends the pending sign-up OTP via [channel]: `email` or `sms`.
  Future<Map<String, dynamic>> sendClientSignupOtp({
    required String email,
    required String channel,
    String? name,
  }) async {
    try {
      final docEmail = email.trim();
      final pendingDoc = await _retryFirestoreOperation(
        () => _firestore.collection('pending_signups').doc(docEmail).get(),
        operationName: 'Get pending signup for OTP delivery',
      );

      if (!pendingDoc.exists) {
        return {
          'success': false,
          'message': 'No pending sign-up found. Please sign up again.',
        };
      }
      ...
      final recipientName = name?.trim().isNotEmpty == true
          ? name!.trim()
          : UserModel.resolveDisplayName(data);

      await _retryFirestoreOperation(
        () => _firestore.collection('pending_signups').doc(docEmail).update({
          'otpDeliveryChannel': channel,
          'expiresAt': FieldValue.serverTimestamp(),
          'verified': false,
        }),
        operationName: 'Update OTP delivery channel',
      );
      ...
      if (channel == 'sms') {
        if (phoneNumber.isEmpty || userId.isEmpty) {
          return {
            'success': false,
            'message': 'Phone number is required to send OTP via SMS.',
          };
        }
        final outcome = await SmsService().sendSignupOtpSms(
          to: phoneNumber,
          body:
        ...
        if (!outcome.isSent) {
          return {
            ...
            'message':
                outcome.error ??
                'Failed to send verification code via SMS. Try email instead.',
          };
        }
        return {
          'success': true,
      ...
      return {
        'success': false,
        'message': 'Invalid delivery channel. Choose email or SMS.',
      };
    } catch (e) {
      if (kDebugMode) {
        debugPrint('❌ Error in sendClientSignupOtp: $e');
      }
      return {
        'success': false,
  ...
  Future<Map<String, dynamic>> attorneySignup({
    ...
    try {
      ...
      try {
        ...
        smsSent = outcome.isSent;
        if (!smsSent && kDebugMode) {
          debugPrint('⚠️ Attorney OTP SMS not delivered: ${outcome.error}');
        }
      } catch (e) {
        if (kDebugMode) {
          debugPrint(
            '⚠️ Could not send attorney OTP via SMS (non-critical): $e',
          );
        }
      }
  ...
  Future<Map<String, dynamic>> verifyOtpAndCreateAccount({
    ...
    try {
      ...
      final firstName = (pendingData['firstName'] as String? ?? '').trim();
      final middleName = (pendingData['middleName'] as String? ?? '').trim();
      final lastName = (pendingData['lastName'] as String? ?? '').trim();
      final displayName = firstName.isNotEmpty || lastName.isNotEmpty
          ...
          : (pendingData['fullName'] as String? ?? '').trim();
      ...
      if (userId == null || userId.isEmpty) {
        return {
          'success': false,
          'message':
              'Could not find the created account. Please login and try again.',
        };
      }
  ...
  Future<Map<String, dynamic>> login({
    ...
    try {
      ...
      // Get user data from Firestore (fewer retries — long loops ANR the UI on mobile).
      ...
      final emailLower = email.toLowerCase().trim();
      final finalRole = resolvePortalRole(emailLower, firestoreRole: role);
      var finalIsVerified = isVerified;
      if (['admin', 'attorney', 'staff'].contains(finalRole)) {
        finalIsVerified = true;
      }

      await repairPortalRoleIfNeeded(
        uid: user.uid,
        email: emailLower,
        firestoreRole: role,
      );

      // Defer email-verification sync so login navigation is not blocked on mobile.
      if (finalRole.toLowerCase() == 'client') {
        unawaited(_syncEmailVerifiedFromAuth(user, finalIsVerified));
      }
      ...
      // Staff: [isActive] false if application not approved yet (self-reg) or admin deactivated.
      if (finalRole.toLowerCase() == 'staff' &&
          userDataMap['isActive'] == false) {
        await _auth.signOut();
        final awaitingApp =
            userDataMap['staffAwaitingApplicationApproval'] == true;
        return {
          'success': false,
          'message': awaitingApp
              ? 'Your staff application is still waiting for administrator approval, or your access has not been activated yet. Sign-in stays disabled until both are done.'
              : 'Your staff account is not enabled for sign-in. If your application was already approved, ask an administrator to enable your user in User Management.',
        };
      }
      ...
      final needsVerification = !rolesThatDontNeedVerification.contains(
      ...
      // Non-blocking: FCM + audit log must not delay navigation (prevents ANR/crash).
      unawaited(_postLoginSideEffects(user.uid, role));
      ...
      switch (e.code) {
        ...
        case 'api-key-not-valid':
          message =
              'Firebase is not configured for this app build. Reinstall the latest '
              'app from your PC (flutter run) and try again.';
          break;
        default:
          final raw = e.message ?? '';
          if (raw.toLowerCase().contains('api key')) {
            message =
                'Firebase connection error. Ensure the app was rebuilt after the '
                'latest update, then try again.';
          } else {
            message = raw.isNotEmpty ? raw : 'Login failed';
          }
      }
      return {'success': false, 'message': message};
    } catch (e) {
    ...
    try {
      ...
      if (pendingSignup.exists) {
        final pendingChannel =
            channel ?? pendingSignup.data()?['otpDeliveryChannel'] as String?;
        if (pendingChannel == 'email' || pendingChannel == 'sms') {
          return sendClientSignupOtp(
            email: docEmail,
      ...
      if (pendingSignup.exists) {
        await _firestore.collection('pending_signups').doc(docEmail).update({
          'otp': otp,
          'expiresAt': FieldValue.serverTimestamp(),
          'verified': false,
          if (channel != null) 'otpDeliveryChannel': channel,
        });
      } else {
        await _firestore
